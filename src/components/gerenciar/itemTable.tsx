...
export const ItemTable = ({ item, onRefresh }: Props) => {
  ...
  const editForm = moduleEntry.renderForm?.({
    isEdit: true,
    defaultValues: item,
    onRefresh,
  });
  ...
  const handleDelete = async () => {
    if (!moduleEntry.deleteItem) {
      return;
    }

    await moduleEntry.deleteItem(id);
    ...
    <TableRow>
      ...
      <TableCell>
        <div className="flex items-center justify-end gap-1">
          {moduleEntry.capabilities.delete && moduleEntry.deleteItem && (
          ...
          {moduleEntry.capabilities.edit && editForm && (
            <ActionDialog
              triggerIcon="edit"
